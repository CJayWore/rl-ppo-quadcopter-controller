...
DEFAULT_COLAB = False
DEFAULT_TRAJECTORY = "hover"  # "circle", "figure8", "hover"
DEFAULT_NUM_MFS = 3
...
def run(
    ...
    for i in range(0, int(duration_sec*env.CTRL_FREQ)):
        ...
        for j in range(num_drones):
            _, _, _ = pid_teacher_ctrl[j].computeControlFromState(
            ...
            ideal_thrust_vector, ideal_torques = pid_teacher_ctrl[j].getLastThrustAndTorques()
        ...
        if i % (env.CTRL_FREQ * 5) == 0:  # Print every 5 seconds
            ...
            if hasattr(ctrl[0], 'pos_error_history') and len(ctrl[0].pos_error_history) > 100:
                    recent_pos_error = np.mean(ctrl[0].pos_error_history[-100:])
                    recent_att_error = np.mean(ctrl[0].att_error_history[-100:])
                    print(f"   📚 学习进度 - 位置误差: {recent_pos_error:.6f}, 姿态误差: {recent_att_error:.6f}")
