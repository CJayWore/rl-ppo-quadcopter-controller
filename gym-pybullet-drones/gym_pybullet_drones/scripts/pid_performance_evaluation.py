...
class PIDPerformanceMetrics:
    ...
    motor_usage_efficiency: List[float] = field(default_factory=list)
    
    # Safety & Robustness
    safety_violations: List[Dict] = field(default_factory=list)
    max_tilt_angles: List[float] = field(default_factory=list)
    altitude_violations: List[float] = field(default_factory=list)
...
class PIDPerformanceEvaluator:
    ...
    def start_evaluation(self, target_pos: np.ndarray) -> None:
        """Start a new evaluation session."""
        self.start_time = time.time()
        self.target_positions = [target_pos] if len(target_pos.shape) == 1 else target_pos
        self.episode_count = 0
        self.episode_data = []
    ...
    def log_step(self, timestamp: float, state: np.ndarray, target_pos: np.ndarray, 
        ...
        # Store target position for this episode (update on each step to ensure we have the latest)
        self.current_episode['target_pos'] = target_pos.copy()
    ...
    def _compute_summary_metrics(self) -> None:
        ...
        if self.metrics.control_energy and self.metrics.position_errors:
            ...
            self.metrics.control_efficiency = 1.0 / (1.0 + mean_energy * mean_error)
    ...
    def generate_performance_report(self) -> str:
        ...
        report_lines = [
            ...
            f"- **Average Control Energy**: {np.mean(self.metrics.control_energy):.2f}",
            f"- **Average Control Smoothness**: {np.mean(self.metrics.control_smoothness):.4f}",
...
Average Control Energy & {np.mean(self.metrics.control_energy):.2f} \\\\
Control Smoothness & {np.mean(self.metrics.control_smoothness):.4f} \\\\
